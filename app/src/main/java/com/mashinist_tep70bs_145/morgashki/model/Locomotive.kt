 ...
 * Табло с произвольным текстом (например маршрутный указатель на ЭМУ),
 * поверх которого можно вписать свою надпись. rect — положение и размер
 * окна табло на фоне (как у обычных слоёв, ширина в % от ширины фона).
 * heightToWidthRatio — во сколько раз высота окна табло меньше его ширины
 * (высота_окна_px / ширина_окна_px), рассчитывается индивидуально под
 * пропорции конкретного фото — НЕ одна и та же для всех локомотивов.
 * maxTextWidthPct — максимальная ширина текста в процентах ширины фона:
 * если введённый текст в выбранном шрифте оказывается шире этого предела,
 * ввод не применяется, чтобы текст не вылезал за рамки табло.
 * textOffsetXRatio — отступ текста от левого края окна табло, в долях
 * ширины окна (0.06 = 6% от ширины табло). textSizeRatio — размер шрифта
 * текста, в долях высоты окна табло. presetRoutes — готовые маршруты,
 * показываемые кнопками в диалоге ввода (специфичны для конкретного МВПС,
 * не глобальные).
 */
data class DisplayBoard(
    ...
    val maxChars: Int = 17,
    val heightToWidthRatio: Float = 0.221f,
    val textOffsetXRatio: Float = 0.06f,
    val textSizeRatio: Float = 0.48f,
